...
export const postNewTransaction = (rawTransaction: {
  date: moment.Moment;
  amount: number;
}) => {
  return axios.post<UnsanitizedTransaction[]>('/transaction', {
...
export const retrieveTransactions = (): Promise<Transaction[]> => {
  return axios.get<UnsanitizedTransaction[]>('/transactions')
      .then(response => {
        return sanitizeTransactions(response.data);
      });
};

interface UnsanitizedTransaction {id: string; date: string; amount: number;}
